...
def _stage_samples_per_epoch(n_samples: int, stage: str, batch_size: int, device: torch.device, cfg: TrainPipelineConfig) -> int:
...
def _train_one_epoch(model: nn.Module, loader: DataLoader, optimizer: torch.optim.Optimizer, ce_loss: nn.Module, scaler: torch.amp.GradScaler | None, use_amp: bool, device: torch.device, cfg: TrainPipelineConfig) -> dict[str, float]:
    model.train()
    loss_sum = 0.0
